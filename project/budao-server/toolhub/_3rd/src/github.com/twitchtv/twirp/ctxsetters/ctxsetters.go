...
func WithRequestIp(ctx context.Context, ip string) context.Context {
	return context.WithValue(ctx, contextkeys.RequestIpKey, ip)
}

func WithSIG(ctx context.Context, sig string) context.Context {
	return context.WithValue(ctx, contextkeys.SIGKey, sig)
}
