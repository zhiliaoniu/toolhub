...
func RequestIp(ctx context.Context) (string, bool) {
	name, ok := ctx.Value(contextkeys.RequestIpKey).(string)
	return name, ok
}
func SIG(ctx context.Context) (string, bool) {
	name, ok := ctx.Value(contextkeys.SIGKey).(string)
	return name, ok
}
