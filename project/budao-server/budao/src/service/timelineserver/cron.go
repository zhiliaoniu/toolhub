...
func UpdateListItems() {
	var err error
	var exist bool
	if exist, err = db.KExists(common.VID_DYNAMIC); err != nil || !exist {
		glog.Debug("no vid need update in %s. err:%v", common.VID_DYNAMIC, err)
		return
	}
...
func RecomposeListitems(vids, questionIds []string, topicIds []string, listItemMap map[string]*pb.ListItem) (listItemMap2 map[string]*pb.ListItem, err error) {
	...
	for _, listItem := range listItemMap {
		...
		if commentItem, ok := vidHotCommentMap[vid]; ok {
...
// UpdateVideoRecommendList update video recommend list
func UpdateVideoRecommendList() {
	...
	for {
		select {
		case <-timer.C:
			timer.Reset(time.Duration(2) * time.Hour)
			for i := 0; i < int(videoTableNum); i++ {
				videoTableName := fmt.Sprintf("video_%d", i)
				glog.Debug("LoadRecommendVIDIntoCache begin")
				err := LoadRecommendVIDIntoCache(videoTableName)
				if err != nil {
					glog.Error("LoadRecommendVIDIntoRedis failed. err:%v", err)
					continue
				}
				glog.Debug("LoadRecommendVIDIntoCache end")
			}
		}
	}
}

// LoadRecommendVIDIntoCache load recommend vid into cache recommendlist
func LoadRecommendVIDIntoCache(tableName string) (err error) {
	execSQL := fmt.Sprintf("select vid, parse_type, state, op_state from %s", tableName)
	...
	parseTypeConf := common.GetConfig().ParseTypeConf
	var vidArr []string
	for rows.Next() {
		var vid uint64
		var state uint32
		var opstate uint32
		var parseType uint32
		rows.Scan(&vid, &parseType, &state, &opstate)
		if state != 2 || opstate != 0 {
			continue
		}
		key := fmt.Sprintf("parse_%d;", parseType)
		if exist := strings.Contains(parseTypeConf, key); !exist {
			continue
		}
		vidArr = append(vidArr, strconv.FormatUint(vid, 10))
	}

	lock.Lock()
	defer lock.Unlock()
	Recommendlist = &vidArr

	return err
}

var (
	// Recommendlist global
	Recommendlist *[]string
	lock          = new(sync.RWMutex)
)

// GetRecommendList get recommendlist from cache
...
func TimerLoadVideoFullInfo() {
	...
	timer := time.NewTimer(time.Duration(30) * time.Minute)
	go func() {
		for {
			select {
			case <-timer.C:
				timer.Reset(time.Duration(30) * time.Minute)
				for i := 0; i < int(videoTableNum); i++ {
					videoTableName := fmt.Sprintf("video_%d", i)
					err := LoadVideoFullInfotoRedis(videoTableName)
					if err != nil {
...
func LoadVideoFullInfotoRedis(tableName string) (err error) {
	...
	glog.Debug("end read video table")
	...
	if common.GetConfig().TimelineConf.ShowQuestion == true {
		...
		glog.Debug("end read question table")
	}
	//------------- end question ------------
	...
	glog.Debug("end read topic table")

	//1: 外显个数
	glog.Debug("begin read hot comment table")
	vidHotCommentArr, err := util.GetVidsHotCommentQuikly(vidArr, 1)
	if err != nil {
	...
	glog.Debug("end read hot comment table")
	...
	glog.Debug("end marshal listItem")
	...
	glog.Debug("update video full info success")
