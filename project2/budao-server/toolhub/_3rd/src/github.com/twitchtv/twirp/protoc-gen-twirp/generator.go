...
func (t *twirp) Generate(in *plugin.CodeGeneratorRequest) *plugin.CodeGeneratorResponse {
	...
	t.registerPackageName("cityhash")
	t.registerPackageName("glog")
...
func (t *twirp) generateImports(file *descriptor.FileDescriptorProto) {
	...
	t.P(`import `, t.pkgs["cityhash"], ` "github.com/zhenjl/cityhash"`)
	t.P(`import `, t.pkgs["glog"], ` "github.com/sumaig/glog"`)
...
func (t *twirp) generateServerRouting(servStruct string, file *descriptor.FileDescriptorProto, service *descriptor.ServiceDescriptorProto) {
	...
	t.P(`  ctx = `, t.pkgs["ctxsetters"], `.WithPackageName(ctx, "`, pkgName, `")`)
	t.P(`  ctx = `, t.pkgs["ctxsetters"], `.WithServiceName(ctx, "`, servName, `")`)
	t.P(`  ctx = `, t.pkgs["ctxsetters"], `.WithRequestIp(ctx, req.Header.Get("Remote_addr"))`)
	t.P(`  ctx = `, t.pkgs["ctxsetters"], `.WithSIG(ctx, req.Header.Get("sig"))`)
	...
	t.P(`    if req.Method =="OPTIONS" {`)
	t.P(`    	resp.Header().Add("Access-Control-Allow-Origin", "*")
...
func (t *twirp) generateServerJSONMethod(service *descriptor.ServiceDescriptorProto, method *descriptor.MethodDescriptorProto) {
	...
	t.P(`  }`)
	t.P()
	t.P(`  // check client signature`)
	t.P(`  sig, _ := `, t.pkgs["twirp"], `.SIG(ctx)`)
	t.P(`  if sig != "" {`)
	t.P(`      buf, _ := `, t.pkgs["proto"], `.Marshal(reqContent)`)
	t.P(`      seed := uint64(307976497148328517)`)
	t.P(`      ch := `, t.pkgs["cityhash"], `.CityHash64WithSeed(buf, uint32(len(buf)), seed)`)
	t.P(`      sigNum, _ := `, t.pkgs["strconv"], `.ParseUint(sig, 10, 64)`)
	t.P(`      if sigNum != ch {`)
	t.P(`          `, t.pkgs["fmt"], `.Printf("req has bad_signature. req:%v, correct_sig:%d\n", req, ch)`)
	t.P(`      } else {`)
	t.P(`          `, t.pkgs["fmt"], `.Printf("req has correct_signature. req:%v\n", req)`)
	t.P(`      }`)
	t.P(`  } else {`)
	t.P(`      `, t.pkgs["fmt"], `.Printf("req has no_signature req:%v\n", req)`)
	t.P(`  }`)
	t.P()
	...
	t.P(`  marshaler := &`, t.pkgs["jsonpb"], `.Marshaler{OrigName: false}`)
...
func (t *twirp) generateServerProtobufMethod(service *descriptor.ServiceDescriptorProto, method *descriptor.MethodDescriptorProto) {
	...
	t.P(`      sigNum, _ := `, t.pkgs["strconv"], `.ParseUint(sig, 10, 64)`)
	t.P(`      if sigNum != ch {`)
	t.P(`          `, t.pkgs["glog"], `.Error("req has bad_signature. req:%v, correct_sig:%d", req, ch)`)
	t.P(`      } else {`)
	t.P(`          `, t.pkgs["glog"], `.Debug("req has correct_signature. req:%v", req)`)
	t.P(`      }`)
	t.P(`  } else {`)
	t.P(`      `, t.pkgs["glog"], `.Error("req has no_signature req:%v", req)`)
	t.P(`  }`)
	t.P()
	t.P(`  // Call service method`)
	t.P(`  var respContent *`, t.goTypeName(method.GetOutputType()))
