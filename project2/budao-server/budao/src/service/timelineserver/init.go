...
var GIOSAuditVids []string

func InitIOSOnlineAuditVids() {
	GIOSAuditVids = make([]string, 0)
	var fileName string
	fileName = common.GetConfig().IOSAuditConf.VidsFileName
	glog.Debug("filename:%s", fileName)
	if fileName == "" {
		fileName = "./conf/ios_audit_vids"
	}
	f, err := os.Open(fileName)
	if err != nil {
		glog.Error("open ios audit vids file failed. filename:%s, err:%v", fileName, err)
		panic(err)
	}
	...
	for {
		...
		if err != nil {
			if err == io.EOF {
				glog.Debug("init ios online audit vid success. GIOSAuditVids.len:%d", len(GIOSAuditVids))
				//glog.Debug("GIOSAuditVids:%v", GIOSAuditVids)
				break
			}
			panic(err)
		}
		line = strings.TrimSpace(line)
		GIOSAuditVids = append(GIOSAuditVids, line)
	}
	base.ShuffleStringArr(GIOSAuditVids)
}
...
func InitVideoDynamicInfo() {
	...
	tableNum := common.GetConfig().DB.MySQL[common.BUDAODB].TableDesc[commentserver.VIDEO_TABLE_PREFIX]
	var num uint64
	for num = 0; num < tableNum; num++ {
		tableName := fmt.Sprintf("%s%d", commentserver.VIDEO_TABLE_PREFIX, num)
		sqlString := fmt.Sprintf("select vid, favor_num, fake_favor_num, comment_num, fake_comment_num, view_num, fake_view_num, share_num, fake_share_num, could_favor, could_comment, could_share from %s where state = 2 and op_state = 0", tableName)
