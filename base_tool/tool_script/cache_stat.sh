#!/bin/bash
# 28-Dec-2014Brendan GreggCreated this.
### default variables
...
do
    case $opt in
        D)inopt_debug=1 ;;
        t)inopt_debugopt_timestamp=1 ;;
        h|?)inopt_debugopt_timestampusage ;;
    esac
done
shift $(( $OPTIND - 1  ))
### option logic
...
while (( !quit && (!opt_duration || secs < duration)  )); do
    ...
    cat /proc/meminfo trace_stat/function* | awk -v debug=$opt_debug '# match meminfo stats:
